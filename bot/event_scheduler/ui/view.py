...
class ScheduleEventEmbed(discord.Embed):
    def __init__(self):
        super().__init__(title="Schedule Event", description="Schedule an event", color=discord.Color.pink())
...
class SelectParticipant(ui.MentionableSelect):
    ...
    async def callback(self, interaction: Interaction):
        ...
        self.embed.add_field(name="Participants", value=", ".join([participant.mention for participant in self.values]))
...
class AddDescriptionModal(ui.Modal): 
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        self.embed.insert_field_at(0, name="Event Name", value=self.name.value, inline=False)
